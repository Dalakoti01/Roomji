...
export async function POST(req) {
  ...
  if (!freeTrialUsed) {
    ...
    existingUser.freeTrial.endDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // example: 7-day trial
    await existingUser.save();
  } else {
  ...
  return NextResponse.json(
    ...
    // ✅ 6. Create new rented property
    const newRoom = new rentedPropertiesModels({
      ...
      area,
      securityDeposit,
      roomPolicies: Array.isArray(roomPolicy)
        ? roomPolicy
        : typeof roomPolicy === "string"
        ? roomPolicy.split(",").map((r) => r.trim())
        : [],
      uniqueCode,
    ...
    await RecentActivities.create({
      ...
      content : `${existingUser.fullName} has posted a new property for rent: ${newRoom.title}.`,
      })
