...
export async function POST(req) {
  ...
  return NextResponse.json(
    {
      message:
        "You have exhausted your free trial. Please buy a subscription to keep posting shops.",
    ...
    const newShop = new shopModels({  
      ...
      category,
      securityDeposit,
      shopPolicies: Array.isArray(shopPolicies)
        ? shopPolicies
        : typeof shopPolicies === "string"
        ? shopPolicies.split(",").map((r) => r.trim())
        : [],
      uniqueCode,
    ...
    await RecentActivities.create({
      ...
      content : `${existingUser.fullName} has posted a new shop: ${newShop.title}.`,
      })
