...
import useGetSingleSellingProperty from "@/hooks/public/useGetSingleSellingProperty";
...
export default function EditSellingProperties() {
  const { id } = useParams();
  const router = useRouter();

  useGetSingleSellingProperty(id);
  ...
  useEffect(() => {
    if (selectedProperty?._id) {
      setForm({
        ...
        policies: selectedProperty.policies || [],
        newPolicy: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `/api/edit/sellingProperty/${id}`,
        payload
      );

      if (data?.success) {
        toast.success("Selling property updated successfully");
        router.back();
      } else {
        toast.error(data?.message || "Failed to update property");
      }
    } catch (error) {
      console.error("UPDATE SELLING PROPERTY ERROR:", error);
      toast.error(
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">
          Edit Selling Property
        </h1>
        <p className="text-gray-500 text-sm">
          Update your property sale details carefully
        </p>
      </div>
      ...
      >
        ...
        {/* Policies */}
        <div>
          <Label className="text-lg">Property Policies</Label>
