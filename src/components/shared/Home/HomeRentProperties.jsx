...
export default function HomeRentProperties({ filters }) {
  ...
  const filteredProperties = React.useMemo(() => {
    if (!rentedProperties) return [];

    return rentedProperties.filter((property) => {
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-700">
              {rentedProperties?.length
                ...
                : "No rented properties available"}
            </h3>
            <p className="text-gray-500 mt-2 max-w-md">
              {rentedProperties?.length
                ? "Try changing your filters or clear them to see more results."
                : "Currently, there are no rented properties listed. Please check back later or refresh to see new updates."}
            </p>
            <button
